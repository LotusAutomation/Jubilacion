...
/*
			...
			if (textElement.getFirstChild() != null) {
				valueField = readJSONFile.formatText(textElement.getFirstChild().getNodeValue().trim());
				if (type.equals(ElementosJson.TYPEDATAMULTILINE)) {
					valueField = readJSONFile.formatText(textElement.getTextContent().trim());
				}
				if (valueFieldJSON.equalsIgnoreCase(valueField)) {
					//texto = "[" + valueFieldJSON + "] SI es igual a [" + valueField + "]";
					bCorrecto =  true;
				} else {
					//texto = "[" + valueFieldJSON + "] NO es igual a [" + valueField + "]";
					bCorrecto = false;
				}
			}
			return bCorrecto;*/
