...
public class ArchivosComprimidos {
	...
	public boolean comparateAttachments(ArrayList<String> filesJSON, ArrayList<String> filesFolder, String filepath,  String universalID, String keyField) {
		...
		attachmentCheck = filesJSON.containsAll(filesFolder) && arrFilesFolder.containsAll(Arrays.asList(listFileJSON));
			...
			if (attachmentCheck == false) {
				...
				String sMensajeAttachments = "[ " + universalID + " ] Attachment XML [ " + sValorAttachmentXml   + " ] DIFERENTE DE JSON [ " + sValorAttachmentJson +" ]";
				ListaDeFallidos.validarFallidos("fallido$-" + sValorAttachmentXml, universalID, keyField, sValorAttachmentJson);
				// ------------Escribir en Archivo de evidencia---------------
					}
	...
	public boolean comparateAttachments(ArrayList<String> filesJSON,String[] filesFolder) {
		...
		//System.out.println("comparateAttachments ---------" + arrFilesFolder.containsAll(Arrays.asList(listFileJSON)));
//		System.out.println("list folder ---------" + arrFilesFolder.toString());
//		System.out.println("containsAll ---------" + filesJSON.containsAll(Arrays.asList(filesFolder)));
//		System.out.println("list json ---------" + filesJSON.toString());
		return filesJSON.containsAll(Arrays.asList(filesFolder)) && arrFilesFolder.containsAll(Arrays.asList(listFileJSON));
	}
	*/
